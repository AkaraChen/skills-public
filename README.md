...
The three `resume-*` skills share the same reader runtime, and each of them carries its own copy under `references/resume-session/` (`CORE.md` + `session_reader.py`), so every skill directory is self-contained — copy the folder alone into your skills directory and it works.
